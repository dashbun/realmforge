...
export const WorldProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateLore = async (id: string, data: Partial<Lore>): Promise<Lore> => {
    ...
    if (loreIndex === -1) throw new Error('Lore entry not found');
    
    const updatedLoreEntry = {
      ...lore[loreIndex],
      ...data,
      updatedAt: new Date(),
    };
    
    const updatedLoreList = [...lore];
    updatedLoreList[loreIndex] = updatedLoreEntry;
    
    setLore(updatedLoreList);
    localStorage.setItem(`realmforge_lore_${currentWorld.id}`, JSON.stringify(updatedLoreList));
